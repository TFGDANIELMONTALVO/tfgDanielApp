...
export function GroupDetail() {
  ...
  const isUserInGroup = useCallback (() => {
    return group.users.some((user) => user._id.toString() === userId.toString())
  }, [group])
  ...
  const usersInGroup = useCallback (() => {
    ...
    return (group.numOfUsers - group.users.length) + "/" + group.numOfUsers + " sitios disponibles"
  }, [group])

  const platformColors = useCallback(() => {
    switch (group.category) {
        case "Disney+":
            return {width: "100%", height: "5rem", background: "linear-gradient(90deg, rgba(0,57,255,1) 0%, rgba(9,13,121,1) 100%)",}
            break;

        case "Netflix Premium":
            return {width: "100%", height: "5rem", background: "linear-gradient(90deg, rgba(255,0,0,1) 0%, rgba(121,9,9,1) 100%)",}
            break;

        default:
            return {width: "100%", height: "5rem", background: "linear-gradient(90deg, rgba(112,9,121,1) 0%, rgba(41,0,135,1) 100%)",}
            break;
    }
  },[group])
  ...
  return (
    <>
      ...
      {
        group ? (
        <Card key={group.category} className="text-center" style={platformColors()}>
          <Card.Body>
      ...
      {group ? (
        <Container key={[group.category, group.ownerId.name, group.serviceUser, group.servicePassword, group.numOfUsers, group.price]}>
            <Row className="mt-5 mb-4 justify-content-center">
              ...
              <Col md="5">
                <p>{isUserInGroup() ? "Utiliza estas credenciales para acceder a la plataforma" : "Accede al grupo para poder ver las credenciales"}</p>
                <h3>Credenciales:</h3>
                <p>
                  <b>Email: {group.serviceUser}</b>
                </p>
                <p>
                  <b>Contraseña: {group.servicePassword}</b>
                </p>
              </Col>
            </Row>
            <Row className="justify-content-center">
              <Col md="5">
                <p>Estado: {isGroupComplete() ? <b>CERRADO</b> : <b>ACTIVO</b>}</p>
                <p>Fecha de creación: {format(new Date(group.createdAt), "dd-MM-yyyy")}</p>
              ...
              <Col md="5" className="text-center">
                <Card bg="light">
                    {
                      isAdminGroup() ? <AdminGroupPanelControl/> : 
                      <Card.Body>
                        <Card.Text>
                          <p>{isUserInGroup() ? "Pagando:" : "En grupo:"}</p>
                          <h3>{group.price}€</h3>
                        </Card.Text>
                        <Button variant={isUserInGroup() ? "danger" : "primary"} type="text" onClick={isUserInGroup() ? onClickExitGroup : onClickJoinGroup}>
                          {
                            isUserInGroup() ? "Abandonar grupo" : "Unirse al grupo"
                          }
                          </Button>
                      </Card.Body>    
                    }
